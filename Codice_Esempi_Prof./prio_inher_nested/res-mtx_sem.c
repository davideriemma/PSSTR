...
/* An adaption of the test res-mtx_sem with two nested critical sections 
by Marcello Cinque */
...
static struct op_buffer test_op_2;
...
void task_func(long tid)
{
	...
	} else  if (tid>1005&&tid<=1010) { // high priority tasks use test_op_2;
		
		int i;

		
	  	RT_PRINTK("task: %d, priority: %d\n", tid, task->priority);
		RT_PRINTK("task: %d, about to lock mutex 2...\n", tid);

		rt_sem_wait(&test_op_2.lock);

		RT_PRINTK("task: %d, mutex 2 locked!\n", tid);
	
		for (i = 0; i < 5; i++) {
			RT_PRINTK("task: %d, loop count %d\n", tid, i);
			test_op_2.t_buffer_data[test_op.t_buffer_pos] = tid + i;
			test_op_2.t_buffer_tid[test_op.t_buffer_pos] = tid;
			test_op_2.t_buffer_pos++;
		}
	  	
		RT_PRINTK("task: %d, about to unlock mutex 2.\n", tid);
		rt_sem_signal(&test_op_2.lock);
		RT_PRINTK("Control task: priority back to %d.\n", control_task.priority);
	} else { // remaining low priority task use test_op
		int i;

		
	  	RT_PRINTK("task: %d, priority: %d\n", tid, task->priority);
		RT_PRINTK("task: %d, about to lock mutex 1...\n", tid);

		rt_sem_wait(&test_op.lock);

		RT_PRINTK("task: %d, mutex 1 locked!\n", tid);
...
void control_func(long tid)
{
	...
	rt_typed_sem_init(&test_op_2.lock, 1, RES_SEM); 
	...
	RT_PRINTK("task: %d, about to unlock mutex 1...\n", tid);
	rt_sem_signal(&test_op.lock);
